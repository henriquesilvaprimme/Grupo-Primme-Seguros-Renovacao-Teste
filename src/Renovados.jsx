...
const LeadsFechados = ({ leads, usuarios, onUpdateInsurer, onConfirmInsurer, onUpdateDetalhes, fetchLeadsFechadosFromSheet, isAdmin, scrollContainerRef }) => {
    ...
    const handleRefresh = async () => {
        setIsLoading(true);
        try {
            // Solicita explicitamente apenas a aba "Renovados"
    ...
    useEffect(() => {
        // Agora assume-se que `leads` já contém apenas os registros da aba "Renovados"
        const fechadosAtuais = Array.isArray(leads) ? leads : [];
        ...
        setMeioPagamento(prevMeioPagamento => {
            ...
            fechadosAtuais.forEach(lead => {
                ...
                if (lead.ID && (!novosMeios[lead.ID] || (apiMeioPagamento !== '' && prevMeioPagamento[lead.ID] === undefined))) {
        ...
        setCartaoPortoNovo(prevCartaoPortoNovo => {
            ...
            fechadosAtuais.forEach(lead => {
                ...
                if (lead.ID && (!novosCartoes[lead.ID] || (apiCartaoPortoNovo !== '' && prevCartaoPortoNovo[lead.ID] === undefined))) {
                                    ...
                                    {!isSeguradoraPreenchida ? (
                                        <button
                                            onClick={async () => {
                                                ...
                                                // Após confirmar, solicita explicitamente a aba "Renovados"
                                                await fetchLeadsFechadosFromSheet && fetchLeadsFechadosFromSheet('Renovados'); 
                                            }}
