...
  // Função para simular a atualização local do lead
  const atualizarAtribuicaoLocal = (leadId, userId) => {
    // Encontra o nome do usuário a partir do ID
    const usuarioSelecionado = usuarios.find(u => u.id === userId);
    const nomeUsuario = usuarioSelecionado ? usuarioSelecionado.nome : '';

    // Chama a função transferirLead (que deve atualizar o estado 'leads' no componente pai)
    // É importante que 'transferirLead' atualize o lead na lista local.
    transferirLead(leadId, userId, nomeUsuario); // Assumindo que transferirLead agora aceita o nome.

    // Opcional: Limpa a seleção após "enviar"
    setSelecionados(prev => {
      const newState = { ...prev };
...
    }
    
    // 1. Simulação de atualização LOCAL
    atualizarAtribuicaoLocal(leadId, userId);

    // 2. Envio para a API (agora com a atualização local já feita)
    const lead = leads.find((l) => l.id === leadId);
    const leadAtualizado = { 
      ...lead, 
      usuarioId: userId,
      // O nome do responsável é importante para o Apps Script
      responsavel: usuarios.find(u => u.id === userId)?.nome || '' 
    };
...
        },
      });
      // Não é necessário chamar fetchLeadsFromSheet(SHEET_NAME) aqui 
      // pois a atualização local já foi feita em handleEnviar.
      // A chamada para 'fetchLeadsFromSheet' pode ser feita posteriormente para garantir a sincronia.
    } catch (error) {
