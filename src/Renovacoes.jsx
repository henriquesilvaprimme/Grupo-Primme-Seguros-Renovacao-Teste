...
const Renovacoes = ({ leads, usuarios, onUpdateStatus, transferirLead, usuarioLogado, fetchLeadsFromSheet, scrollContainerRef }) => {
    ...
    const [filtroData, setFiltroData] = useState(''); // Estado do filtro de Vigência Final (YYYY-MM)
    ...
    const aplicarFiltroNome = () => {
        ...
        setFiltroData(''); // Limpa o filtro de data ao aplicar o filtro de nome
        setDataInput('');
        setFiltroStatus(null);
        setPaginaAtual(1);
    };
    ...
    const aplicarFiltroStatus = (status) => {
        ...
        setFiltroNome('');
        setNomeInput('');
        setFiltroData(''); // Limpa o filtro de data ao aplicar o filtro de status
        setDataInput('');
        setPaginaAtual(1);
    };
    ...
    const gerais = leads.filter((lead) => {
        // 1. Excluir Fechado/Perdido (Regra Invariante)
        if (lead.status === 'Fechado' || lead.status === 'Perdido') return false;

        // 2. Filtro de Status (Prioridade 1, se setado)
        if (filtroStatus) {
        ...
        // 3. Filtro de Nome (Prioridade 2, se setado)
        if (filtroNome) {
        ...
        // 5. Nenhuma filtro de filtroStatus, filtroNome ou filtroData aplicado
        return true;
    });
    ...
    return (
        <div style={{ padding: '20px', position: 'relative', minHeight: 'calc(100vh - 100px)' }}>
            ...
            >
                ...
                {/* --- CONTEINER ISOLADO PARA O SINO E A BOLHA --- */}
                {hasScheduledToday && (
                    <div
                        style={{
                            flex: 1,
