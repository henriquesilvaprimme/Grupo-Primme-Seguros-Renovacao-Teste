...
const Segurados = () => {
 ...
 cliente.vehiclemodel || cliente.Modelo || '',
 ...
 false,
  ...
  const handleEndossar = (segurado, vehicle) => {
    // 1. OBTÉM O ID EXCLUSIVO DA LINHA DA APÓLICE/VEÍCULO
    const uniquePolicyId = obterIDPorVeiculo(segurado, vehicle); [cite_start]// [cite: 80]

    setEndossoData({
      // 2. ENVIA O ID EXCLUSIVO DO VEÍCULO/APÓLICE
      //    O Apps Script usará este ID (junto com o nome) para encontrar a linha EXATA.
      clienteId: uniquePolicyId || segurado.id, // Usa o ID exclusivo, ou o ID do cliente como fallback
    ...
    setShowEndossoModal(true); [cite_start]// [cite: 40]
  };
  // ====================================================================
  ...
  const handleSaveEndosso = async () => {
    ...
    try {
      const payload = {
        ...
        id: endossoData.clienteId, // AGORA ESTE ID DEVE SER O ID EXCLUSIVO DA APÓLICE
 ...
 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white"
                  >
                    {[...Array(12)].map((_, i) => (
                ...
                <div className="flex gap-3 mt-6">
                  ...
                  <button
                    ...
                    className="flex-1 
 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors disabled:bg-blue-400 disabled:cursor-not-allowed"
                  >
